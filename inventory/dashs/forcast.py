...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        for p in list(set(_all_products) - set(products_with_existing_forecast)):
            _bulk_forecast = []
            for c in _all_circuits:  # _all_customers[:min(len(_all_customers), 30000)]:
                # Extract filtered data as dataframe
                sub_df = df.loc[(df['product'] == p) & (df['circuit'] == c)]
                ...
                if len(sub_df.index) > 10:  # _forecast_periods:
                    print('sub_df\n', sub_df)
                    # Get circuit id
                    ...
                    forecast = _get_forecasts(sub_df[['ds', 'y']], holidays=None, add_country_holidays='MA',
                    ...
                    self.stdout.write(
                        'Forecasts for [product: %s, circuit: %s] is added to the batch with version %s' % (
                        p, c, version_id))
                    self.stdout.write(
                    ...
                    self.stdout.write(self.style.WARNING(
                        'Forecasts for [product: %s, circuit: %s] is skipped because of insufficient data' % (p, c)))
                    self.stdout.write(
        ...
        self.stdout.write(self.style.SUCCESS(
            '%s forecasts are saved successfully in %s sec . %s forecasts skipped cause of insufficient data' % (
            _added_forecasts, time.time() - start_time, _skipped_forecasts)))
