...
def group_data_by_category_2(queryset, x_key, y_keys):
    '''
    input = queryset([
        {'x_key': 2020, 'y_keys[0]': 0, 'y_keys[1]': 2},
        {'x_key': 2021, 'y_keys[0]': 1, 'y_keys[1]': 0},
        {'x_key': 2022, 'y_keys[0]': 1, 'y_keys[1]': 2},
    ])
    output = dict({
        'y_keys[0]': [
            {'x': [2020, 2021, 2022]},
    ...
    '''
    chartdata = []
    data = queryset.values(x_key, *y_keys, named=True)
    for y_key in y_keys:
        chartdata.append(
            go.Bar(
                x=data.values_list(x_key, flat=True),
                y=data.values_list(y_keys, flat=True),
                name=y_key,
            )
        )
    return chartdata
