...
def plot_order_count_figure(selected_products, selected_categories, selected_customers, selected_status, start_date,
    ...
    if new_df.size != 0:
        ...
        new_df['Not Delivered'] = new_df.apply(
            lambda row: 1 if row.delivered_quantity == 0 or row.delivered_quantity is None or row.delivered_at is None else 0, axis=1)
        ...
        new_df['Delivered In Time'] = new_df.apply(
            lambda
                row: 1 if row.delivered_quantity >= row.ordered_quantity and row.ordered_at >= row.delivered_at else 0,
            axis=1)
...
def plot_order_count_figure(selected_products,selected_categories,selected_customers,selected_status,start_date,end_date):
    ...
    if new_df.size != 0:
        ...
        def otif(row):
            sum = row['Not Delivered']+row['Partially Delivered']+row['Delivered In Time']+row['Delivered Not In Time']
            if sum !=0:
...
def plot_order_count_figure(selected_products, selected_categories, selected_customers, selected_status, start_date,
    ...
    if new_df.size != 0:
        ...
        new_df['Delivered In Time'] = new_df.apply(
            lambda
                row: 1 if row.delivered_quantity >= row.ordered_quantity and row.ordered_ >= row.delivered_at else 0,
            axis=1)
        new_df['Delivered Not In Time'] = new_df.apply(
            lambda
                row: 1 if row.delivered_quantity >= row.ordered_quantity and row.ordered_at < row.delivered_at else 0,
