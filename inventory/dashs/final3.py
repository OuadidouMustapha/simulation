                            ...
                            dbc.Row([
                                ...
                                dbc.Col([     
                                    html.Div([
                                        html.Div([
                                            dash_utils.get_filter_dropdown(
                                                dropdown_reference_list_id, div_reference_list_id, checkbox_reference_list_id, _all_stock_checks,'Date',select_all=False,)
                                            ])
                                    ]) 
                                ], sm=12, md=6, lg=6),
                            ]),
...
def make_aggregate_figure(selected_products,selected_statuts,selected_locations,selected_categories,groupe_by,start_date,end_date,dates, n_clicks,rows, value_risque_rupture,value_bon_stock):
    ...
    if(groupe_by=='Categorie') : 
        ...
        results = StockCheck.objects.filter(
            ...
            Q(check_date__lte=end_date)).values('product','check_date').order_by('check_date').annotate(sum=Sum('quantity'))
    ...
    #---------------------------------------------------{Table}------------------------------------------------------
            ...
            print(value_risque_rupture,value_bon_stock,'199999999999999999999999941')
            ...
            if value_risque_rupture!=None and value_bon_stock!=None :
                ...
                fig = go.Figure(data=[go.Pie(labels=labels,values=values, pull=[0.1, 00.1, 0.2, 0.1])])
                fig.update_traces(
...
def set_stock_images_reference_options(selected_products,selected_status, selected_locations,selected_categories,selected_references,groupe_by,start_date,end_date):
    if groupe_by == 'Location-Product':
        ...
        stock_images = StockCheck.objects.distinct('check_date').all().filter(
            Q(product__in=selected_products)&
            Q(status__in=selected_status)&
            Q(location__in=selected_locations)&
            Q(product__category__in=selected_categories)&
            Q(check_date__gte=start_date)&
            Q(check_date__lte=end_date)).values('product','check_date').order_by('check_date')
