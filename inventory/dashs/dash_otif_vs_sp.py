...
from stock.models import Product, ProductCategory, OrderDetail,Supplier
...
app = DjangoDash('otif_supplier', add_bootstrap_links=True)
_prefix = 'otif_supplier'
...
figure_supplier_id = dash_utils.generate_html_id(_prefix, 'figure_supplier_id')
...
def plot_OrderDetail_count_by_custmoer_figure(selected_products, selected_categories, selected_suppliers,
    ...
    print("--- %s seconds ------- rrad data frame  ---" % (time.time() - start_time))
    ...
    #
    # new = df.groupby(
    #     by=['product_id', 'supplier_id', 'order__id'],
    #     as_index=False
    # ).agg({
    ...
    #             row: 1 if row.delivered_quantity >= row.ordered_quantity and row.desired_at < row.delivered_at else None,
    #         axis=1)
    #     df_data = new_df.groupby(['supplier_id']).agg({
    #         'Not Delivered': 'sum',
...
def plot_order_count_figure(selected_products, selected_categories, selected_suppliers, selected_status, start_date,end_date):
...
def plot_order_count_figure(selected_products, selected_categories, selected_suppliers, selected_status, start_date,
    ...
    #
    # # new = df.groupby(
    # #     by=['product_id', 'supplier_id', 'order__id','order__reference'],
    # #     as_index=False
    # # ).agg({
    # #     'order__delivery__deliverydetail__delivered_quantity': 'sum',
    # #     'order__delivery__delivered_at': 'max',
...
def plot_pie_statuts_product_figure(selected_products, selected_categories, selected_suppliers, selected_status,
    ...
    return figure_pie_orderDetail,figure_pie_order
